...
from tests.test_trainers.params_funcs import LGB_fixed_params_func, LGB_search_params_func, Sklearn_fixed_params_func, \
    Sklearn_search_params_func, Torch_fixed_params_func, Torch_search_params_func
...
PARAMS_FUNCS = {'LGB': {'search': LGB_search_params_func, 'fixed': LGB_fixed_params_func},
                'Sklearn': {'search': Sklearn_search_params_func, 'fixed': Sklearn_fixed_params_func},
                'Torch': {'search': Torch_search_params_func, 'fixed': Torch_fixed_params_func}}
RUN_PARAMS = {'neptune': {'neptune_run_params': {'name': 'Test',
                                                 'description': '1'}},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ['Torch', {'nn_model_module': nn_model, 'data_loaders_module': data_loaders,
                   'metrics_callback_module': metrics_callback, 'use_average_epochs_on_test_fold': True}],
        ['Torch', {'nn_model_module': nn_model, 'data_loaders_module': data_loaders,
                   'metrics_callback_module': metrics_callback, 'use_average_epochs_on_test_fold': False}]
    ])
class TestTrainer:

    def init_trainer(self, trainer_name, trainer_specific_params, validation_metric, direction, optimization_metric):

        return TRAINERS[trainer_name](validation_metric=validation_metric, direction=direction,
                                      saved_files_path=r'temp_files/',
                                      optimization_metric=optimization_metric, **trainer_specific_params)

    @pytest.mark.dependency(name='test_init_trainer')
    def test_init_trainer(self, trainer_name, trainer_specific_params):
        self.init_trainer(trainer_name, trainer_specific_params, VALIDATION_METRICS[0], 'minimize',
    ...
    def test_init_run(self, trainer_name, trainer_specific_params, logging_server, upload_files, logging_server_auth):
        trainer = self.init_trainer(trainer_name, trainer_specific_params, VALIDATION_METRICS[0], 'minimize',
                                    OPTIMIZATION_METRICS[0])
        run_params = add_server_auth(logging_server, RUN_PARAMS[logging_server], logging_server_auth)
    ...
    @pytest.mark.dependency(name='test_train', depends=['test_init_trainer', 'test_init_run'])
    def test_train(self, trainer_name, trainer_specific_params, validation_metric, direction, optimization_metric,
                   logging_server, sampler_name, search_hparams, n_trials, logging_server_auth):
        trainer = self.init_trainer(trainer_name, trainer_specific_params, validation_metric, direction,
                                    optimization_metric)
