...
class SklearnInference(_BaseModelInference):
    ...
    def get_params_model(self, metric: str = 'metric_mean_cv', step: Union[int, str] = 'best',
        ...
        """Get logged parameters, hyperparameters, metrics and scikit-learn model from particular step and fold in run.
        ...
        self.downloaded_params['validation_metric'] = self.server_inference.get_run_params()
        if step == 'best':
        ...
        if trained_model:
            trained_model_path = self.server_inference.get_file(
                ...
                r'{}/downloaded_models/model_trial_{}_fold_{}.pickle'.format(self.downloaded_files_path, step,
                                                                             fold_num))
            with open(trained_model_path, 'rb') as f:
                self.downloaded_params['trained_model'] = pickle.load(f)

        else:
            sklearn_estimator_path = self.server_inference.get_file(
                'saved_utils/sklearn_estimator.pickle',
                r'{}/downloaded_utils/sklearn_estimator.pickle'.format(
                    self.downloaded_files_path))
            with open(sklearn_estimator_path, 'rb') as f:
                self.downloaded_params['model'] = pickle.load(f)(**self.downloaded_params['params']['model_params'])
