...
from mlexp.inference._base_inference import SERVER_INFERENCES, _BaseModelInference
...
class TorchInference(_BaseModelInference):
    ...
    def __init__(self, downloaded_files_path: str, inference_server_params: dict,
        ...
        self.server_inference = SERVER_INFERENCES[server](inference_server_params, downloaded_files_path)
    ...
    def get_params_model(self, metric: str = 'metric_mean_cv', step: Union[int, str] = 'best',
        ...
        self.downloaded_params['use_average_epochs_on_test_fold'] = self.server_inference.get_run_params()
        if step == 'best':
            step = self.server_inference.get_best_step(self.downloaded_params['direction'], metric)
        self.downloaded_params['step'] = step
        self.downloaded_params['metric'] = self.server_inference.get_metric(step, metric)
        ...
        optuna_study_path = self.server_inference.get_file(
            ...
            r'{}/downloaded_studies/optuna_study_{}.pickle'.format(self.downloaded_files_path, step))
        with open(optuna_study_path, 'rb') as f:
            self.downloaded_params['optuna_study'] = pickle.load(f)
        initial_min_epoches = self.downloaded_params['params']['trainer_params']['min_epochs']
        initial_max_epoches = self.downloaded_params['params']['trainer_params']['max_epochs']

        nn_model_module_path = self.server_inference.get_file('saved_utils/nn_model_module.py',
                                                              r'{}/downloaded_utils/nn_model_module.py'.format(
                                                                  self.downloaded_files_path))
        data_loaders_module_path = self.server_inference.get_file('saved_utils/data_loaders_module.py',
        ...
        metrics_callback_module_path = self.server_inference.get_file(
            ...
            r'{}/downloaded_utils/metrics_callback_module.py'.format(self.downloaded_files_path))
        self.downloaded_params['nn_model'] = self._load_module('nn_model_module', nn_model_module_path).nn_model
        self.downloaded_params['data_loaders'] = self._load_module('data_loaders_module',
                                                                   data_loaders_module_path).train_val_data_loaders
        self.downloaded_params['metrics_callback'] = self._load_module('metrics_callback_module',
                                                                       metrics_callback_module_path).MetricsCallback

        if trained_model:

            weigths_args_path = self.server_inference.get_file(
                r'saved_models/model_trial_{}_fold_{}.ckpt'.format(step, fold_num),
                r'{}/downloaded_models/model_trial_{}_fold_{}.ckpt'.format(self.downloaded_files_path, step, fold_num))
            self.downloaded_params['trained_model'] = self.downloaded_params['nn_model'].load_from_checkpoint(
                weigths_args_path,
                **nn_model_params)

            if self.downloaded_params['use_average_epochs_on_test_fold']:
                self.downloaded_params['params']['trainer_params']['min_epochs'] = self.downloaded_params['params'][
                    'validation_mean_epochs']
                self.downloaded_params['params']['trainer_params']['max_epochs'] = self.downloaded_params['params'][
                    'validation_mean_epochs']
            ...
            self.downloaded_params['trainer'] = pl.Trainer(
                **self.downloaded_params['params']['trainer_params'],
                callbacks=[
                    pl.callbacks.early_stopping.EarlyStopping(self.downloaded_params['params']['EarlyStopping_params']),
                    metrics_callback])

            self.downloaded_params['params']['trainer_params']['min_epochs'] = initial_min_epoches
            self.downloaded_params['params']['trainer_params']['max_epochs'] = initial_max_epoches
