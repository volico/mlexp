...
class SklearnTrainer(_BaseTrainer, _BaseLogger):
    ...
    def __init__(self,
        ...
        super().__init__(direction, saved_files_path, optimization_metric, validation_metric, model_type='sklearn')
    ...
    def _initiate_neptune_run(self, neptune_run_params: dict, upload_files: Iterable[str] = []) -> None:
        ...
        file_name = r'{}/saved_utils/sklearn_estimator.pickle'.format(self.saved_files_path)
    ...
    def _run_iteration(self, X, y, cv, params, trial_number):
        ...
        sklearn_scoring = make_scorer(self.validation_metric, greater_is_better=is_higher_better)

        estimator = self.sklearn_estimator
        estimator_initialised = estimator(**params['model_params'])
        cv_results = cross_validate(estimator_initialised,
                                    X,
                                    y,
                                    cv=cv,
        ...
        model_file_paths = _save_models(cv_results['estimator'], self.saved_files_path, trial_number)
