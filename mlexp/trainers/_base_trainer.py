...
class _BaseTrainer(ABC):
    ...
    def __int__(self, direction: str,
    ...
    def train(self, X: np.ndarray, y: np.ndarray, cv: list, n_trials: int,
              params_func: Callable[[optuna.trial.Trial], dict], sampler: optuna.samplers.BaseSampler):
        ...
        assert isinstance(sampler, optuna.samplers.BaseSampler), 'Sampler must be initiated sampler from optuna.samplers'
        for fold in cv:
            assert len(
                fold) == 2, 'Each fold in cv must contain 2 sublists: one corresponding train indexes, and one corresponding test indexes'

        self.study = optuna.create_study(sampler=sampler,
                                         direction=self.direction,
                                         study_name='optuna_study')

        self.study.optimize(lambda trial: self._objective(trial, X, y, cv, params_func),
                            n_trials=n_trials, callbacks=[])
    ...
    @abstractmethod
    def _run_iteration(self, X: np.ndarray, y: np.ndarray, cv: list, params: dict, trial_number: int) -> dict:
    ...
    def _objective(self, trial, X, y, cv, params_func):
        ...
        results_dict = self._run_iteration(X,
                                           y,
                                           cv,
                                           params,
                                           trial.number)
        ...
        study_path = r'{}/saved_studies/optuna_study_{}.pickle'.format(self.saved_files_path, trial.number)
