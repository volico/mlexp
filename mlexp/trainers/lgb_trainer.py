...
class LgbTrainer(_BaseTrainer, _BaseLogger):
    ...
    def __init__(self,
        ...
        assert type(use_average_n_estimators_on_test_fold) == bool, 'log_metric_curves must be bool'

        super().__init__(direction, saved_files_path, optimization_metric, validation_metric, model_type='lgb')

        self.use_average_n_estimators_on_test_fold = use_average_n_estimators_on_test_fold
    ...
    def _initiate_neptune_run(self, neptune_run_params: dict, upload_files: Iterable[str] = []) -> None:
        ...
        self.run['use_average_n_estimators_on_test_fold'] = self.use_average_n_estimators_on_test_fold
    ...
    def _initiate_mlflow_run(self, tracking_uri: str, experiment_name: str, mlflow_run_params: dict,
        ...
        mlflow.log_param('use_average_n_estimators_on_test_fold', self.use_average_n_estimators_on_test_fold)

    def _scoring_wrapper(self, validation_metric, direction):
        def lgb_scoring(y_hat, data, validation_metric=validation_metric, direction=direction):
    ...
    def _train_validation(self, X, y, params, cv, lgb_scoring):
        ...
        validation_results = lgb.cv(params=params['model_params'],
        ...
        return (validation_results['val_score-mean'],
    ...
    def _train_test(self, X, y, params, cv, lgb_scoring):
        ...
        train_data = lgb.Dataset(X_train,
                                 y_train, **params['lgb_data_set_params'])
        test_data = lgb.Dataset(X_test,
                                y_test, **params['lgb_data_set_params'])
        test_results = {}
        trained_test_model = lgb.train(params=params['model_params'],
                                       train_set=train_data,
    ...
    def _run_iteration(self, X: np.ndarray, y: np.ndarray, cv: list, params: dict, trial_number: str) -> dict:
        ...
        metric_mean_validation_curve, metric_std_validation_curve, n_estimators, trained_validation_models = \
        ...
        metric_test_curve, trained_test_model = self._train_test(X, y, params, cv, lgb_scoring)
        params['model_params']['n_estimators'] = initial_n_estimators

        model_file_paths = _save_models([*trained_validation_models, trained_test_model], self.saved_files_path,
                                        trial_number)

        metric_curves = {'mean_validation_metric': metric_mean_validation_curve,
