...
class TorchTrainer(_BaseTrainer, _BaseLogger):
    ...
    def __init__(self,
        ...
        assert type(nn_model_module) == types.ModuleType, 'nn_model_module must be module'
        assert type(data_loaders_module) == types.ModuleType, 'data_loaders_module must be module'
        assert type(metrics_callback_module) == types.ModuleType, 'metrics_callback_module must be module'
        assert isinstance(validation_metric, Callable)
        assert type(use_average_epochs_on_test_fold) == bool, 'log_metric_curves must be bool'

        super().__init__(direction, saved_files_path, optimization_metric, validation_metric, model_type='torch')
    ...
    def _initiate_neptune_run(self, neptune_run_params: dict, upload_files: Iterable[str] = []) -> None:
        ...
        self.run['use_average_epochs_on_test_fold'] = self.use_average_epochs_on_test_fold
        ...
        self.run[file_name.split(self.saved_files_path)[-1]].upload(file_name, wait=True)
        os.remove(file_name)

        data_loaders_module_code_lines = inspect.getsource(self.data_loaders_module)
        file_name = r'{}/saved_utils/data_loaders_module.py'.format(self.saved_files_path)
        with open(file_name, 'w') as f:
            f.write(data_loaders_module_code_lines)
        self.run[file_name.split(self.saved_files_path)[-1]].upload(file_name, wait=True)
        os.remove(file_name)

        metrics_callback_module_code_lines = inspect.getsource(self.metrics_callback_module)
        file_name = r'{}/saved_utils/metrics_callback_module.py'.format(self.saved_files_path)
        with open(file_name, 'w') as f:
            f.write(metrics_callback_module_code_lines)
        self.run[file_name.split(self.saved_files_path)[-1]].upload(file_name, wait=True)
        os.remove(file_name)

    def _initiate_mlflow_run(self, tracking_uri: str, experiment_name: str, mlflow_run_params: dict,
        ...
        mlflow.log_param('use_average_epochs_on_test_fold', self.use_average_epochs_on_test_fold)

        nn_model_module_code_lines = inspect.getsource(self.nn_model_module)
        file_name = r'{}/saved_utils/nn_model_module.py'.format(self.saved_files_path)
        with open(file_name, 'w') as f:
            f.write(nn_model_module_code_lines)
        mlflow.log_artifact(file_name, file_name.split(self.saved_files_path)[-1].split('/')[1])
        os.remove(file_name)
    ...
    def _train_model(self, X, y, fold, params):
        ...
        train_loader, val_loader = self.data_loaders_module.train_val_data_loaders(X,
                                                                                   y,
                                                                                   fold,
                                                                                   **params['data_loaders_params'])

        trainer = pl.Trainer(**params['trainer_params'],
                             callbacks=[pl.callbacks.early_stopping.EarlyStopping(**params['EarlyStopping_params']),
                                        metrics_callback]
                             )

        my_model = self.nn_model_module.nn_model(**params['model_params'], validation_metric=self.validation_metric)
    ...
    def _run_iteration(self, X, y, cv, params, trial_number):
        ...
        for fold_num, fold in enumerate(cv):
            ...
            if fold_num == len(cv) - 1:
                ...
                if self.use_average_epochs_on_test_fold == True:
                    mean_number_of_iterations = int(round(np.mean(number_of_iterations)))
            ...
            model_file_path = r'{}/saved_models/model_trial_{}_fold_{}.ckpt'.format(self.saved_files_path,
                                                                                    trial_number,
                                                                                    fold_num)
            ...
            metric_curves['fold_{}_metric'.format(str(fold_num))] = [str(x) for x in metrics_callback.get_metric()]

        metric_curves_file = _save_metric_curves(metric_curves, self.saved_files_path, trial_number)
        ...
        return {'metrics':
                ...
                'file_paths':
                    [*model_file_paths, metric_curves_file],
                'params': params}
