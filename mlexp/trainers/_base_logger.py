...
class _BaseLogger(ABC):
    ...
    @abstractmethod
    def _initiate_mlflow_run(self, tracking_uri, experiment_name, mlflow_run_params, upload_files):
        return

    def init_run(self, logging_server: Literal["neptune", "optuna"], upload_files: Iterable[str] = [],
                 **run_params) -> str:
        ...
        if logging_server == 'neptune':
            ...
            run_id = self._base_initiate_neptune_run(upload_files=upload_files, **run_params)
            self._initiate_neptune_run(upload_files=upload_files, **run_params)

        elif logging_server == 'mlflow':

            run_id = self._base_initiate_mlflow_run(upload_files=upload_files, **run_params)
    ...
    def _base_initiate_neptune_run(self, neptune_run_params, upload_files=[]):
        ...
        file_name = r'{}/saved_utils/validation_metric.pickle'.format(self.saved_files_path)
        ...
        if len(upload_files) > 0:
            for file_name in upload_files:
                self.run['saved_files/{}'.format(file_name.split('/')[-1])].upload(file_name, wait=True)

        self.logging_server = 'neptune'

        return self.run['sys']['id'].fetch()

    def _base_initiate_mlflow_run(self, experiment_name, upload_files=[], tracking_uri=None, mlflow_run_params=None):
        ...
        if (len(upload_files) > 0) and (type(upload_files != str)):
            for file_name in upload_files:
                mlflow.log_artifact(file_name, 'saved_files/{}'.format(file_name.split('/')[-1]))
    ...
    def _log_params_mlflow(self, params):
        ...
        try:
            ...
            with open(r'{}/saved_utils/params.json'.format(self.saved_files_path)) as file:
    ...
    def _log_files_neptune(self, file_paths):
        for file_path in file_paths:
            self.run[file_path.split(self.saved_files_path)[-1]].upload(file_path, wait=True)
            os.remove(file_path)

    def _log_files_mlflow(self, file_paths):
        for file_path in file_paths:
            mlflow.log_artifact(file_path, file_path.split(self.saved_files_path)[-1].split('/')[1])
            os.remove(file_path)
